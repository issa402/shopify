"""
USE CASE: Real disable run from SharedServices or another direct-assume account.

Run this when your terminal credentials can directly assume each member account's
ManagedInstance-CrossAccountExecutionRole. The script disables the exact keys in
the workbook by setting them to Inactive.
"""
...
RESULTS_CSV = "disabled_sharedservices_one_hop_results.csv"  # Output report.
...
def assume_member_account(account_id):
    ...
    response = sts.assume_role(  # Ask STS for member-account credentials.
        ...
        RoleSessionName=f"disable-unused-keys-{account_id}",  # CloudTrail name.
    )
